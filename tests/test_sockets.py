...
class sockets(BrowserCore):
  ...
  def test_sockets_echo(self, harness_class, port, args):
    ...
    if harness_class == WebsockifyServerHarness and WINDOWS:
      self.skipTest('Python pickling bug causes WebsockifyServerHarness to not work on Windows.')

    harness = harness_class(test_file('sockets/test_sockets_echo_server.c'), args, port)
    with harness:
  ...
  def test_sockets_echo_pthreads(self):
    harness = CompiledServerHarness(test_file('sockets/test_sockets_echo_server.c'), [], 49161)
    with harness:
  ...
  @no_windows('Python pickling bug causes WebsockifyServerHarness to not work on Windows')
  def test_nodejs_sockets_echo_subprotocol(self):
    # Test against a Websockified server with compile time configured WebSocket subprotocol. We use a Websockified
    # server because as long as the subprotocol list contains binary it will configure itself to accept binary
    # This test also checks that the connect url contains the correct subprotocols.
    harness = WebsockifyServerHarness(test_file('sockets/test_sockets_echo_server.c'), [], 59166)
    with harness:
    ...
    ''')
    harness = WebsockifyServerHarness(test_file('sockets/test_sockets_echo_server.c'), [], 59168)
    with harness:
